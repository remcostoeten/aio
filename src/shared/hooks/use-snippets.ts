...
import { queries, mutations } from '@/shared/services/supabase'
...
export function useSnippets(userId: string) {
  ...
  const searchSnippets = useCallback(async (query: string) => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to search snippets'))
    } finally {
      setLoading(false)
    }
  }, [userId])
  ...
  // Create snippet
  const createSnippet = useCallback(async (snippet: Omit<Snippet, 'id' | 'created_at' | 'updated_at'>) => {
    try {
